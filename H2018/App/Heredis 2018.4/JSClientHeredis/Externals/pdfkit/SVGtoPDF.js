...
var SVGtoPDF = function(doc, svg, x, y, options) {
    ...
    doc.writeToGroup = function(group) {
      var prevGroup = this._currentGroup || null,
    ...
    function validateMatrix(m) {
      var m0 = validateNumber(m[0]), m1 = validateNumber(m[1]), m2 = validateNumber(m[2]),
    ...
    function parseTranform(v) {
      ...
      while (temp = parser.match(/^([A-Za-z]+)[(]([^(]+)[)]/, true)) {
        var func = temp[1], nums = [], parser2 = new StringParser(temp[2].trim()), temp2;
    ...
    var SvgPath = function(d) {
      SvgShape.call(this);
      var ArgumentsNumber = {A:7,a:7, C:6,c:6, H:1,h:1, L:2,l:2, M:2,m:2, Q:4,q:4, S:4,s:4, T:2,t:2, V:1,v:1, Z:0,z:0};
      var command, value, values, argsNumber, temp, parser = new StringParser((d || '').trim());
    ...
    var SvgShape = function(commands) {
      var pathCommands = this.pathCommands = [], startX = 0, startY = 0, currX = 0, currY = 0, lastCom, lastCtrlX, lastCtrlY;
      ...
      this.Q = function(cx, cy, x, y) {
        var c1x = currX + 2 / 3 * (cx - currX), c1y = currY + 2 / 3 * (cy - currY),
      ...
      this.A = function(rx, ry, rotAngle, arcLarge, arcSweep, x, y) { // From PDFKit
        var xInit = currX, yInit = currY, xEnd = x, yEnd = y;
      ...
      // Shape properties
      var pathSegments = null; Object.defineProperty(this, 'pathSegments', {get: function() {
        if (pathSegments !== null) {return pathSegments;}
        var currX = 0, currY = 0, startX = 0, startY = 0, segments = [];
        for (var i = 0; i < pathCommands.length; i++) {
          var command = pathCommands[i][0], values = pathCommands[i].slice(3), segment;
          switch(command) {
            case 'M':
              segment = null;
      ...
      var boundingBox = null; Object.defineProperty(this, 'boundingBox', {get: function() {
      ...
      this.insertInDocument = function() {
        for (var i = 0; i < pathCommands.length; i++) {
          var command = pathCommands[i][0], values = pathCommands[i].slice(3);
          switch(command) {
      ...
      this.getSubPaths = function() {
        var subPaths = [], shape = new SvgShape();
        for (var i = 0; i < pathCommands.length; i++) {
          var data = pathCommands[i], command = pathCommands[i][0];
          if (command === 'M' && i !== 0) {
            subPaths.push(shape);
            shape = new SvgShape();
          }
      ...
      this.getMarkers = function() {
        ...
        for (var i = 0; i < subPaths.length; i++) {
          var subPath = subPaths[i], subPathMarkers = [];
    ...
    var SvgElem = function(obj, inherits) {
      ...
      this.resolveUrl = function(value) {
        ...
        var id = temp[1] || temp[2] || temp[3] || temp[4];
        if (id) {
      ...
      this.computeLength = function(value, percent, initial) {
        var parsed = this.parseLength(value);
        if (parsed) {
      ...
      this.computeLengthList = function(value, percent) {
        var parsed = this.parseLengthList(value), result = [];
        ...
        switch (key) {
            ...
            result = (function() {
              ...
              if (parsed) {
                if (parsed.unit === '%' || parsed.unit === 'em' || parsed.unit === 'ex') {
                  var parentValue = (this.getInherit() ? this.getInherit().get(key) : keyInfo.initial);
          ...
          case 'stroke-dasharray':
            result = (function() {
              ...
              var dasharray = this.computeLengthList(value, this.getViewport()),
                  sum = 0;
          ...
          case 'stroke-width': case 'stroke-miterlimit':
            result = (function() {
              var parsed = this.computeLength(value, this.getViewport());
          ...
          case 'stroke-dashoffset':
            result = (function() {
              var result = this.computeLength(value, this.getViewport());
              if (result != null) {
                if (result < 0) { // fix for crbug.com/660850
                  var dasharray = this.get('stroke-dasharray');
                  for (var j = 0; j < dasharray.length; j++) {result += dasharray[j];}
                }
                return result;
              }
            }).call(this);
    ...
    var SvgElemStylable = function(obj) {
      ...
      this.clip = function() {
        if (this.get('clip-path') !== 'none') {
          var clipPath = new SvgElemClipPath(this.get('clip-path'), null, this.getBoundingBox());
      ...
      this.mask = function() {
        if (this.get('mask') !== 'none') {
          var mask = new SvgElemMask(this.get('mask'), null, this.getBoundingBox());
          mask.drawInDocument();
          return true;
        }
      };
        ...
        for (var i = 0; i < children.length; i++) {
          if (children[i].get('display') !== 'none') {
            if (typeof children[i].getBoundingShape === 'function') {
              var childShape = children[i].getBoundingShape()
    ...
    var SvgElemGradient = function(obj, inherits, bBox, gOpacity) {
      ...
      this.getGradient = function(isClip, isMask) {
        ...
        if (this.name === 'linearGradient') {
          var x1 = this.getLength('x1', (bBoxUnits ? 1 : this.getVWidth()), 0),
              x2 = this.getLength('x2', (bBoxUnits ? 1 : this.getVWidth()), 1),
              y1 = this.getLength('y1', (bBoxUnits ? 1 : this.getVHeight()), 0),
              y2 = this.getLength('y2', (bBoxUnits ? 1 : this.getVHeight()), 0);
          grad = doc.linearGradient(x1, y1, x2, y2);
        } else {
          var x2 = this.getLength('cx', (bBoxUnits ? 1 : this.getVWidth()), 0.5),
        ...
        for (var i = 0; i < children.length; i++) {
          ...
          var opacity = color[3] * child.get('stop-opacity') * gOpacity;
          if (opacity < 1) {
            if (isMask) {
              color[0] *= opacity;
              color[1] *= opacity;
        ...
        if (!isClip) {
          ...
          for (var j = 0; j < subPaths.length; j++) {
            if (stroke && subPaths[j].totalLength === 0) {
              var LineWidth = this.get('stroke-width'), LineCap = this.get('stroke-linecap');
              if ((LineCap === 'square' || LineCap === 'round') && LineWidth > 0) {
                var x = subPaths[j].boundingBox[0],
          ...
          if (this.get('marker-start') !== 'none') {
            var marker = new SvgElemMarker(this.get('marker-start'), null, markersPos[0], this.get('stroke-width'));
            marker.drawInDocument();
          }
          if (this.get('marker-mid') !== 'none') {
            for (var i = 1; i < markersPos.length - 1; i++) {
              var marker = new SvgElemMarker(this.get('marker-mid'), null, markersPos[i], this.get('stroke-width'));
              marker.drawInDocument();
            }
          }
          if (this.get('marker-end') !== 'none') {
            var marker = new SvgElemMarker(this.get('marker-end'), null, markersPos[markersPos.length - 1], this.get('stroke-width'));
            marker.drawInDocument();
          }
          if (group) {
            doc.closeGroup(group);
    ...
    var SvgElemRect = function(obj, inherits) {
      ...
      if (w > 0 && h > 0) {
        if (rx && ry) {
          ...
          var k = (4 / 3) * (Math.sqrt(2) - 1), cx = rx * (1.0 - k), cy = ry * (1.0 - k);
    ...
    var SvgElemPolyline = function(obj, inherits) {
      SvgElemBasicShape.call(this, obj, inherits);
      var points = this.parseLengthList(this.attr('points'));
      this.shape = new SvgShape();
      for (var i = 0; i < points.length - 1; i+=2) {
        var x = this.computeUnits(points[i].value, points[i].unit, this.getVWidth()),
    ...
    var SvgElemPolygon = function(obj, inherits) {
      ...
      this.shape = new SvgShape();
      for (var i = 0; i < points.length - 1; i+=2) {
        var x = this.computeLength(points[i].value, points[i].unit, this.getVWidth()),
    ...
    var SvgElemTextPath = function(obj, inherits) {
      SvgElemTextContainer.call(this, obj, inherits);
      this.allowedChildren = ['tspan', '#text'];
      var pathObj = this.getUrl('href') || this.getUrl('xlink:href');
    ...
    var SvgElemText = function(obj, inherits) {
      ...
      (function (textParentElem) {
        ...
        function getTextPos(font, size, text) {
          var unit = size / 1000, fontascent = getAscent(font, size), fontdescent = getDescent(font, size);
        ...
        function doAnchoring() {
          if (currentChunk.length) {
            ...
            var anchordx = {'startltr': 0, 'middleltr': 0.5, 'endltr': 1, 'startrtl': 1, 'middlertl': 0.5, 'endrtl': 0}[currentAnchor + currentDirection] * width || 0;
        ...
        function adjustLength(pos, length) {
          var firstChar = pos[0], lastChar = pos[pos.length - 1],
          ...
          for (var i = 0; i < children.length; i++) {
            ...
            switch(childElem.name) {
              case 'tspan': case 'textPath':
                ...
                break;
              case '#text':
                var rawText = childElem.textContent, renderedText = rawText, words;
        ...
        function textOnPath(currentElem) {
          var pathElem = currentElem.path;
          if (pathElem) {
            var pathObject = pathElem.shape.clone().transform(pathElem.get('transform')),
            ...
            for (var j = 0; j < currentElem._pos.length; j++) {
              ...
              var charMidX = textOffset + currentElem._pos[j].x * pathLengthScale + 0.5 * currentElem._pos[j].width;
            ...
            var endPoint = pathObject.getPointAtLength(pathComputedLength);
      ...
      this.drawInDocument = function(isClip, isMask) {
        function recursive(elem) {
          var fill = elem.getFill(isClip, isMask),
                ...
                if (fill || stroke || isClip) {
                  ...
                  for (var j = 0, pos = childElem._pos; j < pos.length; j++) {
                    if (!pos[j].hidden && pos[j].width !== 0) {
                      var cos = Math.cos(pos[j].rotate), sin = Math.sin(pos[j].rotate), skew = (elem._font.fauxitalic ? -0.25 : 0);
